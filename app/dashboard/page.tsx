...
const Dashboard = () => {
  ...
  const { data: newArrivals = { products: [] }, isLoading: isLoadingNewArrivals } = useFetchProductsQuery({ limit: 4 });
  const { data: popularCategory = { products: [] }, isLoading: isLoadingPopularCategory } = useFetchCategoryQuery("laptops");
  ...
  useEffect(() => {
    ...
    fetch(`https://api.unsplash.com/photos/random?query=fashion&orientation=landscape&client_id=${UNSPLASH_KEY}`)
      ...
      .catch(() => setHeroImage("/placeholder.svg"));
  ...
  return (
    <div className="container mx-auto px-6 py-8 space-y-12"> {/* Increased horizontal padding */}
      {/* Hero Section */}
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
          {isLoadingNewArrivals
            ...
            : (newArrivals.products || []).map((product: any) => <ProductCard key={product.id} {...product} />)}
        </div>
      ...
      <section className="bg-muted rounded-xl p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {isLoadingPopularCategory
            ? Array(3)
              .fill(0)
              .map((_, i) => <Skeleton key={i} className="h-[200px]" />)
        ...
        <div className="text-center mt-8">
          <Button variant="outline" onClick={() => router.replace("/categories/laptops")} size="lg">
            Explore Electronics
            <ArrowRight className="ml-2 h-4 w-4" />
          </Button>
        </div>
      ...
      <section className="bg-primary text-primary-foreground rounded-xl p-8">
        <div className="flex flex-col md:flex-row items-center justify-between">
          ...
          <Image
            src="https://plus.unsplash.com/premium_photo-1728717859117-0c8f45e2dae5?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NDl8fG9mZmVyfGVufDB8fDB8fHww"
            width={300}
            height={300}
